...
- [x] azureVMでデプロイ
- [ ] 未来の自分へのメッセージ機能：<br>
step1:ユーザーは普通の星より見た目が違う星（光が強い、色が違うなど）を発見、クリックすると未来の自分へのメッセージ入力欄が表示される。<br>
step2:ユーザーが未来の自分へのメッセージを入力し、その星は画面上から消える（書いた文言は、未来メッセージ用の新しいテーブルに保存）。<br>
step3:ユーザーが次の日記を書いた時、もしスライダーがオールマイナス（気分がダウンしている）だったら、未来の自分への星が表示され（流れ星のようなアニメーション）、ユーザーがその星をクリックしたら過去自分のメッセージが見られる。<br>
