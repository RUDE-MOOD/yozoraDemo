...
export const useTutorialStore = create((set, get) => ({
  ...
  registerTutorialStar: (starId) => {
    ...
    try {
      const existing = JSON.parse(
        localStorage.getItem("pending_tutorial_cleanup") || "[]",
      );
      existing.push(starId);
      localStorage.setItem(
        "pending_tutorial_cleanup",
  ...
  abortTutorial: async () => {
    ...
    if (tutorialStarIds.length > 0) {
      const { removeStarsByIds } = await import("./useStarStore").then((m) =>
        m.useStarStore.getState(),
      );
