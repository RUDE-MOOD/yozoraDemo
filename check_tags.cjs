...
for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith('//') || line.trim().startsWith('{/*') || line.trim() === '*/' || line.trim() === '/*') continue;
    ...
    while ((match = regex.exec(line)) !== null) {
        ...
        if (fullTag.startsWith('</')) {
            if (stack.length > 0 && stack[stack.length - 1].name === tagName) {
                ...
                console.log(`Mismatch at line ${i + 1}: found closing </${tagName}>, but expected </${stack.length > 0 ? stack[stack.length - 1].name : 'none'}>. Pending stack: ${stack.map(s => s.name + ':' + s.line).join(', ')}`);
            }
        } else {
            // It's an opening tag
