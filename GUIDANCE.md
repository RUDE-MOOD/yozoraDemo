...
### パフォーマンス系
- [x] `<PerformanceMonitor>`を導入し、重い時に解像度(DPR)を下げたりエフェクトの品質を落とす「動的パフォーマンス最適化」
- [x] 星々の描画を1回のDraw Callに圧縮する「インスタンス化（Instanced Rendering）」対応
- [x] 重い`<Billboard>`コンポーネントを排除し、専用シェーダーでの計算を大幅削減
