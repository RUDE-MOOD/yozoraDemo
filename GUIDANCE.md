...
- [x] 日記を書き終えたら、カメラが出来立ての星へ移動し、その後星がフェードイン表示される
- [x] azureVMでデプロイ
- [x] 未来の自分へのメッセージ機能：<br>
...
- [ ] 普通の星が星座の星に変わる流れ：星座のテーブルを作って、supabaseにjsonBデータ型で一つの星座（星座の属性は指定しておく必要がる、例：しし座なら楽しい星の星座）の全ての星の座標（一つの星座に何個の星が良いかまだ決めてない）を予め保存し、似たような感情かつその感情は積極的（スライダー全部50以上）な星が一定数集まったら、だんだん星座の星の座標に移動させ、星座の星に変わる。
- [x] スライダーからの数値を再加工することでより正確な感情を判定する
- [x] Supabaseから返される英語の認証エラーメッセージをわかりやすい日本語に変換
- [x] ユーザーが発射した星の下の日付を表示/非表示にするトグル機能
...
- **Instancing（インスタンス化描画）**: 数百から数千に及ぶ大量の星の描画を「1回のDraw Call」に圧縮するため、`@react-three/drei`の`<Instances>`を使用して大きなパフォーマンス最適化を実施済み。新規追加の星（動的）は一時的に個別描画され、アニメーション終了と共にインスタンス（静的）へ自動吸収される。
- **SingleStarMaterial**: インスタンス描画に適合(`USE_INSTANCING`)するよう調整された円形の光核と十字フレアのシェーダー。ワールド座標由来のランダム値を使ってそれぞれの星が独立して瞬くアニメーションを実行。
- **UIとの連動**: 常にカメラを向く高負荷なBillboardは排除されたが、超軽量なテキストコンポーネントが日付を表示し（設定からの表示切替トグル対応）、クリックで詳細モーダルが開く仕組み。
...
- **Components**:
  ...
  - **User Menu**: 画面左下のユーザーアイコン。プロフィール・マイセイザ・テーマ・ログ・設定・開発者モードへのアクセス。
  - **Diary Modal**: 5つの気分スライダー（0~100）と「今日のいいこと」3つの入力欄。スマホではステップ分割（スライダー → テキスト入力）、PCでは2カラム表示。
  - **Star Detail Modal**: 星をクリックすると表示。感情分析結果・励ましメッセージ・今日のいいこと・座標、そして自動抽出された**タグ(Tag)**などを確認できる。
  - **Log Views Modal (ログ)**: 過去の星（日記）をカレンダー形式で振り返るビュー。日ごとの感情の推移や「今日のいいこと」の数を色付きのドットで視覚的に確認できる。
  - **Constellation Modal (マイセイザ)**: 完成した、または進行中の星座（特定の感情軸が集まった星のグループ）を一覧表示し、詳細情報を確認できるUI。
  - **Dev Mode Modal (開発者モード)**: `localStorage`をハックしてアプリ内の時間を進め（タイムスキップ）、1日1回の制限を無視して連続で日記をテストできるデバッグツール。
  - **Theme Selection Modal**: カラーテーマ（3色）と背景タイプ（クラシック/ネビュラ）の切り替え。
- **App Integration**: `<Canvas>`の外側に配置され、3D シーンの上に重なって表示される。

## 自動抽出タグ (Tag)
「今日のいいこと」に入力されたテキストや、全体的な感情スコアをGemini APIが解析し、短いキーワード（タグ）を自動生成する機能。
生成されたタグは `StarDetailModal` 等で `#ハッシュタグ` 形式で星と一緒に表示され、一目でその日のハイライトを振り返ることができる。
...
ユーザーの認証状態や、基本的なUI設定の管理。

| データ/メソッド | 種類 | 説明 |
|---|---|---|
| `user` | state | 現在のユーザー情報（ID・Email等） |
| `session` | state | Supabaseのセッション情報 |
| `showStarDate` | state | 星の下に生成日時の日付を表示するかどうかのグローバルフラグ |
| `setUser(user)` | method | ユーザー情報をセット |
| `setSession(session)` | method | セッション情報をセット |
| `toggleShowStarDate()` | method | 日付の表示/非表示フラグを切り替え |
